...
def parse_sections(summary_text):
    ...
    if '###SECTION###' in summary_text:
        ...
        for idx, raw_section in enumerate(raw_sections):
            ...
            if not content:
                print(f"Skipping empty section {idx}")
                continue
            ...
            print(f"Section {len(sections)}: {title[:50]}")
    ...
    print(f"Legacy parsing created {len(sections)} sections")
    return sections
        ...
        while True:
            ...
            if not session["introduced"]:
                ...
                intro_prompt = (
                    ...
                    "Say you will help explain the medical report section by section."
                )
                ...
                try:
                    ...
                    print("Generated introduction response...")
                    await websocket.send_json({"type": "message", "text": response.text})
                    
                    print(f"Introduction complete")
            ...
            if session["current_section"] >= len(session["sections"]):
                ...
                try:
                    ...
                    import traceback
                    error_details = traceback.format_exc()
                    error_msg = str(e) if str(e) else "Unknown Gemini API error sp3"
            ...
            try:
                ...
                print("Generated section response...")
                full_response = response.text
                await websocket.send_json({"type": "message", "text": full_response})
                ...
                print(f"Successfully generated section {session['current_section']}/{len(session['sections'])}")
                
            except Exception as e:
                import traceback
                error_details = traceback.format_exc()
                error_msg = str(e) if str(e) else "Unknown Gemini API error sp 1"
